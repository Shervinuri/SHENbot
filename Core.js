    ...
    <div class="main-wrapper">
        <div id="app-container">
            ...
            <div id="input-area">
                ...
                <textarea id="message-input" placeholder="یا انگشتی تایپ کن یا میک بزن بگو..." rows="1"></textarea>
            </div>
        </div>
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    deferredPrompt = e;
    // Show the install button/offer the install prompt after a short delay
    setTimeout(() => {
        if (deferredPrompt) {
...
class ChatInterface {
    ...
    // Energy transfer animation for send
    createEnergyTransfer() {
        ...
        energy.classList.add('energy-transfer');
        ...
        const animate = (timestamp) => {
            ...
            const springProgress = 1 - Math.exp(-6 * progress) * Math.cos(12 * Math.PI * progress);
            
            const currentX = startX + (endX - startX) * springProgress;
            const currentY = startY + (endY - startY) * springProgress;
            
            energy.style.left = `${currentX}px`;
            energy.style.top = `${currentY}px`;
            energy.style.opacity = `${1 - progress}`;
            energy.style.transform = `scale(${1 + 0.5 * (1 - progress)})`;
    ...
    async sendMessage(messageText) {
        ...
        this.createEnergyTransfer(); // Trigger energy transfer animation
